 ...
 */
inline bool criteria_1(const Delaunay m_dela, const Delaunay::Facet f, const double parameter)
{
...
inline double criteria_2(const Delaunay m_dela, const Delaunay::Facet f)
{
    Delaunay::Cell_handle c1 = f.first;
    Delaunay::Cell_handle c2 = m_dela.mirror_facet(f).first;
    int i = f.second;
...
public:
    ...
    inline static void add_from_criteria_cell(Delaunay &m_dela,
        SoT inside) {
        std::clog << "Add from criteria" << std::endl;
...
{
public :
    MedialFeatureField(const Delaunay &m_dela, double parameter = 1.0) : m_dela(m_dela), parameter(parameter)
    {
